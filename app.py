...
def add_task():
    ...
    if request.method == 'POST':
        ...
        if 'taskList' in request.form:
            ...
            return render_template('index.html', task=task, subTasks=subTasks, tasks=tasks, id_task=id_task)
        # Ajout d'une tache        
            ...
            try:
                ...
                tasks = Task.query.filter_by(creator=id_user).order_by(Task.date_created).all()
                return render_template('index.html', task=task, tasks=tasks, id_task=id_task)
            except:
                db.session.rollback()
                task = Task.query.get(id_task)
                subTasks = SubTask.query.filter_by(id_task=id_task).all()
                ...
                return render_template('index.html', task=task, subTasks=subTasks, tasks=tasks, id_task=id_task, errorMessage=errorMessage)
